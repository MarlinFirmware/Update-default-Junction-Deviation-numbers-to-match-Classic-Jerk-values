...
def Upload(source, target, env):
    ...
    def _FilterFirmwareFiles(FirmwareList, UseLongFilenames):
        ...
        for FWFile in FirmwareList:
            ...
            if UseLongFilenames:
                ...
                Space = FWFile.find(" ")
                if Space >= 0:
                    Space = FWFile.find(" ", Space + 1)
                if Space >= 0:
                    FWFile = FWFile[Space + 1 :]
    ...
    upload_firmware_source_path = (
        os.path.join(env["PROJECT_BUILD_DIR"], env["PIOENV"], f"{env['PROGNAME']}.bin")
        if "PROGNAME" in env
        else str(source[0])
    )
    ...
    try:
        ...
        protocol.send_ascii(
            "M117 Firmware uploaded" if transferOK else "M117 Firmware upload failed"
        )
