...
def apply_opt(name, val, conf=None):
    ...
    for file in ("Configuration.h", "Configuration_adv.h"):
        ...
        for i in range(len(lines)):
            ...
            if match and match[4].upper() == name.upper():
                ...
                if val in ("on", "", None):
                    newline = re.sub(
                        r"^(\s*)//+\s*(#define)(\s{1,3})?(\s*)", r"\1\2 \4", line
                    )
                elif val == "off":
                    # TODO: Comment more lines in a multi-line define with \ continuation
                    newline = re.sub(
                        r"^(\s*)(#define)(\s{1,3})?(\s*)", r"\1//\2 \4", line
                    )
                else:
    ...
    if not found:
        ...
        with fullpath.open(encoding="utf-8") as f:
            ...
            lines.insert(
                linenum,
                f"{prefix}#define {added:30} // Added by config.ini {currtime}\n"
            )
...
def disable_all_options():
    # Create a regex to match the option and capture parts of the line
    regex = re.compile(
        r"^(\s*)(#define\s+)([A-Z0-9_]+\b)(\s?)(\s*)(.*?)(\s*)(//.*)?$", re.IGNORECASE
    )
